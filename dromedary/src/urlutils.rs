...
pub fn file_relpath(base: &str, path: &str) -> Result<String> {
    ...
    let base: PathBuf = crate::osutils::path::normpath(local_path_from_url(base)?);
    let path: PathBuf = crate::osutils::path::normpath(local_path_from_url(path)?);

    let relpath = crate::osutils::path::relpath(base.as_path(), path.as_path());
    if relpath.is_none() {
