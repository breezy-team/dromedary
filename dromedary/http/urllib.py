...
evil_logger = logging.getLogger("dromedary.evil")
    ...
    def finish(self):
        ...
        if not self.isclosed():
            ...
            while data and self.length:
                ...
                pending += len(data)
            if pending:
                logger.debug("%s bytes left on the HTTP socket", pending)
            self.close()
        return pending
...
class AbstractHTTPConnection:
    ...
    def _mutter_connect(self):
        ...
        logger.debug("* About to connect() to %s", netloc)
...
class HTTPSConnection(AbstractHTTPConnection, http.client.HTTPSConnection):  # type: ignore
    ...
    def connect_to_origin(self):
        ...
        if cert_reqs == ssl.CERT_NONE:
            logger.warning("Not checking SSL certificate for %s", host)
            ca_certs = None
        else:
        ...
        if exc_type == socket.gaierror:
            ...
            if first_try:
                ...
                if exc_type in (http.client.BadStatusLine, http.client.UnknownProtocol):
                    ...
                    my_exception = InvalidHttpResponse(
    ...
    def do_open(self, http_class, request, first_try=True):
        ...
        if convert_to_addinfourl:
            ...
            if debug_logger.isEnabledFor(logging.DEBUG):
                ...
                logger.debug("< %s %s %s", version, resp.code, resp.msg)
...
class ProxyHandler(urllib.request.ProxyHandler):
    ...
    def set_proxy(self, request, type):
        ...
        parsed_url = _mod_dromedary.ConnectedTransport._split_url(proxy)
...
class AbstractAuthHandler(urllib.request.BaseHandler):
    ...
    def auth_required(self, request, headers):
        ...
        if not server_headers:
            ...
            logger.debug("%s not found", self.auth_required_header)
            return None
...
class NegotiateAuthHandler(AbstractAuthHandler):
    ...
    def _auth_match_kerberos(self, auth):
        ...
        if ret < 0:
            logger.debug("authGSSClientStep failed: %d", ret)
            return None
