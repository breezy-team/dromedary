...
logger = logging.getLogger("dromedary.webdav.tests.dav_server")
...
class TestingDAVRequestHandler(http_server.TestingHTTPRequestHandler):
    ...
    def do_PUT(self):
        ...
        path = self.translate_path(self.path)
        logger.debug(f"do_PUT rel: [{self.path}], abs: [{path}]")
        ...
        try:
            logger.debug(f"do_PUT will try to open: [{path}]")
            ...
            logger.debug(f"do_PUT got: [{e!r}] while opening/seeking on [{self.path}]")
        ...
        try:
            ...
            return
        f.close()
        logger.debug(f"do_PUT done: [{self.path}]")
    ...
    def do_MKCOL(self):
        ...
        """
        path = self.translate_path(self.path)
        logger.debug(f"do_MKCOL rel: [{self.path}], abs: [{path}]")
        try:
            os.mkdir(path)
    ...
    def do_COPY(self):
        ...
        logger.debug(f"urlparse: ({url_to}) [{rel_to}]")
        logger.debug(f"do_COPY rel_from: [{self.path}], rel_to: [{rel_to}]")
    ...
    def do_DELETE(self):
        ...
        """
        path = self.translate_path(self.path)
        logger.debug(f"do_DELETE rel: [{self.path}], abs: [{path}]")
        try:
    ...
    def do_MOVE(self):
        ...
        logger.debug(f"urlparse: ({url_to}) [{rel_to}]")
        logger.debug(f"do_MOVE rel_from: [{self.path}], rel_to: [{rel_to}]")
        abs_from = self.translate_path(self.path)
        abs_to = self.translate_path(rel_to)
