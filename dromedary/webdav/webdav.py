...
logger = logging.getLogger("dromedary.webdav")
    ...
    def endDocument(self):
        ...
        if not self.expected_content_handled:
            raise transport_errors.InvalidHttpResponse(
                self.url, msg="Unknown xml response"
            )
    ...
    def put_bytes_non_atomic(
        ...
        try:
            ...
            if not create_parent_dir:
                raise
            parent_dir = posixpath.dirname(relpath)
            if parent_dir:
...
"""
    ...
    def _append_by_head_put(self, relpath, bytes):
        ...
        if code == 404:
            ...
            if relpath_size == 0:
                logger.debug("if %s is not empty, the server is buggy", relpath)
        if relpath_size:
