...
fn map_urlutils_error_to_pyerr(e: dromedary::urlutils::Error) -> PyErr {
    match e {
        ...
        dromedary::urlutils::Error::NotLocalUrl(url) => InvalidURL::new_err(("Not a local url", url)),
        dromedary::urlutils::Error::UrlNotAscii(url) => InvalidURL::new_err(("URL not ascii", url)),
        dromedary::urlutils::Error::InvalidUNCUrl(url) => InvalidURL::new_err(("Invalid UNC URL", url)),
        ...
        dromedary::urlutils::Error::PathNotChild(path, start) => PathNotChild::new_err((path, start)),
        dromedary::urlutils::Error::UrlTooShort(url) => PyValueError::new_err(("URL too short", url)),
    }
}
...
fn join_segment_parameters(url: &str, parameters: HashMap<String, String>) -> PyResult<String> {
    let parameters = parameters
        ...
        .collect();
    dromedary::urlutils::join_segment_parameters(url, &parameters).map_err(map_urlutils_error_to_pyerr)
}

#[pyfunction]
fn local_path_from_url(url: &str) -> PyResult<String> {
    let path = dromedary::urlutils::local_path_from_url(url).map_err(map_urlutils_error_to_pyerr)?;
...
fn win32_local_path_from_url(url: &str) -> PyResult<String> {
    let path =
        dromedary::urlutils::win32::local_path_from_url(url).map_err(map_urlutils_error_to_pyerr)?;
...
fn posix_local_path_from_url(url: &str) -> PyResult<String> {
    let path =
        dromedary::urlutils::posix::local_path_from_url(url).map_err(map_urlutils_error_to_pyerr)?;
