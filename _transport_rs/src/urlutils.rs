...
/// drive-letter-aware split itself lives in the Rust library; this is just
/// the boundary check that turns the malformed-URL case into a Python
/// exception, which is platform-gated to match breezy.
...
fn split(url: &str, exclude_trailing_slash: Option<bool>) -> PyResult<(String, String)> {
    ...
    Ok(dromedary::urlutils::split(
        url,
        exclude_trailing_slash.unwrap_or(true),
    ))
}
...
fn strip_trailing_slash(url: &str) -> PyResult<String> {
    validate_win32_file_url(url)?;
    Ok(dromedary::urlutils::strip_trailing_slash(url).into_owned())
}
...
fn basename(url: &str, exclude_trailing_slash: Option<bool>) -> PyResult<String> {
    validate_win32_file_url(url)?;
    Ok(dromedary::urlutils::basename(
        url,
        exclude_trailing_slash.unwrap_or(true),
    ))
}
