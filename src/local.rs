...
impl Transport for LocalTransport {
    ...
    fn stat(&self, relpath: &UrlFragment) -> Result<Stat> {
        ...
        // We work on the encoded bytes of the OsStr so this stays
        // WTF-8-safe on Windows and UTF-8-safe on Unix.
        let path = {
            let b = path.as_path().as_os_str().as_encoded_bytes();
            if let Some(stripped) = b.strip_suffix(b"/") {
                // SAFETY: we only stripped an ASCII byte; the rest of the
                // encoding is intact.
                PathBuf::from(unsafe { OsStr::from_encoded_bytes_unchecked(stripped) })
            } else {
                path
            }
        };
    ...
    fn abspath(&self, relpath: &UrlFragment) -> Result<Url> {
        ...
        let path = crate::osutils::path::normpath(path);
    ...
    fn readlink(&self, relpath: &UrlFragment) -> Result<String> {
        ...
        Ok(escape(target.as_os_str().as_encoded_bytes(), None))
    }
    ...
    fn symlink(&self, source: &UrlFragment, link_name: &UrlFragment) -> Result<()> {
        ...
        #[cfg(windows)]
        {
            // On Windows `symlink_file` / `symlink_dir` require distinguishing
            // file vs directory targets and typically Administrator privilege
            // or Developer Mode. Emulate the common case (file symlink).
            // TODO(windows): pick `symlink_dir` when the source is a directory.
            let _ = source_rel;
            let _ = link_name;
            Err(Error::Io(std::io::Error::new(
                std::io::ErrorKind::Unsupported,
                "symlink is not implemented on Windows yet",
            )))
        }
    }
    ...
    fn list_dir(&self, relpath: &UrlFragment) -> Box<dyn Iterator<Item = Result<String>>> {
        ...
        Box::new(
            entries
                ...
                .map(|entry| {
                    entry
                        .map(|entry| escape(entry.file_name().as_os_str().as_encoded_bytes(), None))
                }),
        )
    }
